...
public class BookApplication {
    ...
    public static void main(String[] args){
        ...
        BookDao bookDAO = new ListBookDao(sortSpecificationCreator, searchSpecificationCreator);
        DaoListFiller filler = new DaoListFiller();
        try {
            run(acquirer, bookDAO, filler);
        } catch (ModelException | IOException | DataException e) {
            LOGGER.error(e.getMessage(), e);
        }
    }

    private static void run(DataAcquirer acquirer, BookDao bookDAO, DaoListFiller filler) throws IOException, ModelException, DataException {
